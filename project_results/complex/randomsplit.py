...
def create_csv_with_random_correct_predictions(filtered_ranks_path, output_path, sample_size=50):
    '''
...
# Filtering rows where the tail prediction is correct (tail_rank == 1)
        ...
        sampled_predictions[['head', 'relation', 'tail']].to_csv(output_path, index=False)
