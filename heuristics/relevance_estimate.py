...
def compute(dataset: Dataset):
    ...
    for relation in dataset.relations:
        ...
        samples_containing_relation = relation_2_samples[relation]
        relation_2_count[relation] = len(samples_containing_relation)

        for sample in samples_containing_relation:
            one_step_relpaths = extract_one_step_relpaths(sample, entity_2_samples)

            for one_step_relpath in one_step_relpaths:
                relation_2_relpath_counts[relation][one_step_relpath] += 1
                relation_2_relpathset[relation].add(one_step_relpath)
                all_relpaths.add(one_step_relpath)

            two_step_relpaths = extract_two_step_relpaths(sample, entity_2_samples)
            for two_step_relpath in two_step_relpaths:
                relation_2_relpath_counts[relation][two_step_relpath] += 1
                relation_2_relpathset[relation].add(two_step_relpath)
                all_relpaths.add(two_step_relpath)

            three_step_relpaths = extract_three_step_relpaths(sample, entity_2_samples)
            for three_step_relpath in three_step_relpaths:
                relation_2_relpath_counts[relation][three_step_relpath] += 1
                relation_2_relpathset[relation].add(three_step_relpath)
                all_relpaths.add(three_step_relpath)
    ...
    for relation in dataset.relations:
        ...
        for relpath in all_relpaths:
            numerator = float(relpath_counts[relpath])
