...
from dataset import Dataset, KelpieDataset, ALL_DATASET_NAMES
...
from perturbation import kelpie_perturbation_combination
...
perturbed_list, skipped_list = kelpie_perturbation_combination.remove_combination(dataset=original_dataset,
                                                                                  sample_to_explain=kelpie_sample,
                                                                                  samples = kelpie_train_samples,
                                                                                  kelpie_entity_id = kelpie_entity_id,
                                                                                  n=2,
                                                                                  budget=100)

perturbed_random_list, skipped_random_list = kelpie_perturbation_combination.remove_combination_random(dataset=original_dataset,
                                                                                  sample_to_explain=kelpie_sample,
                                                                                  samples = kelpie_train_samples,
                                                                                  kelpie_entity_id = kelpie_entity_id,
                                                                                  n=2,
...
for i in range(len(perturbed_random_list)):
    samples = perturbed_random_list[i]

    skipped_samples = skipped_random_list[i]
    skipped_facts = []
    for s in skipped_samples:
        print(kelpie_dataset.entity_id_2_name[s[0]])
        print(kelpie_dataset.relation_id_2_name[int(s[1])])
