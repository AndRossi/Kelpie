...
if args.mode == "sufficient":
    ...
    original_scores, original_ranks, original_predictions = predict_samples_in_batches(original_model,
                                                                                       samples_to_convert)

    new_model = TransE(dataset=new_dataset, hyperparameters=hyperparameters, init_random=True)  # type: TransE
    ...
    with open("output_end_to_end_skipping_random_facts.csv", "w") as outfile:
    ...
    # new_scores, new_ranks, new_predictions = new_model.predict_samples(numpy.array(samples_to_explain_with_long_explanations))
    new_scores, new_ranks, new_predictions = predict_samples_in_batches(new_model, numpy.array(
        samples_to_explain_with_long_explanations))
