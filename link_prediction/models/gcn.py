...
class CompGCN(nn.Module):
    def __init__(
        ...
        if not layer_dropout:
            self.layer_dropout = [0.3] * len(layer_size)
        else:
            self.layer_dropout = layer_dropout

        print(self.layer_dropout)
...
class CompGCN_ConvE(nn.Module):
    ...
    def forward(self, graph, sub, rel, obj_lis=None):
        # get sub_emb and rel_emb via compGCN
        n_feats, r_feats = self.compGCN_Model(graph)
        sub_emb = n_feats[sub, :]
        rel_emb = r_feats[rel, :]

        # print(sub.size(), n_feats.size(), sub_emb.size(), rel_emb.size())
        ...
        if obj_lis:
            # compute score
            x = th.mm(x, n_feats[obj_lis, :].transpose(1, 0))
            # add in bias
            x += self.bias[obj_lis].expand_as(x)
        else:
            # compute score
