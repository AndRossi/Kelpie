...
for (head, relation, tail) in facts_to_explain:
    ...
    outlines = []
    print("FACT: " + ";".join([head, relation, tail]))
    ...
    random_sample_index = random.choice(range(0, len(kelpie_train_samples)))
    cur_kelpie_train_samples = numpy.delete(kelpie_train_samples, random_sample_index, axis=0)
    skipped_sample = kelpie_train_samples[random_sample_index]
    ...
    translated_skipped_fact = [skipped_fact[0] if skipped_fact[0] != "kelpie" else original_dataset.entity_id_2_name[original_entity_id],
                               skipped_fact[1],
                               skipped_fact[2] if skipped_fact[2] != "kelpie" else original_dataset.entity_id_2_name[original_entity_id]]
    print("SKIPPING " + ";".join(translated_skipped_fact) + "\t" + "\t".join(
    ...
    outlines.append("SKIPPING " + ";".join(translated_skipped_fact) + "\t" + "\t".join(
                                                     [str(cur_direct_score),
                                                     str(cur_inverse_score),
                                                     str(cur_head_rank),
                                                     str(cur_tail_rank),
