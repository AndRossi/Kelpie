...
class SufficientRulesExtractor:
    ...
    def extract_rules(self,
        ...
        # map each sample to its global X
        # map each sample to a map that says, for each entity to convert, the individual X (this is just for logging
        # use global X instead of coverage
    ...
    def extract_rules_long_1(self, samples_to_add: list):
        ...
        for i, entity_to_convert in enumerate(self.entities_to_convert):
            ...
            for r_added_sample in r_sample_2_detailed_results:
                ...
                relevance = float(base_pt_target_entity_rank - pt_target_entity_rank)/float(base_pt_target_entity_rank-1)
                original_added_sample = Dataset.replace_entity_in_sample(sample=r_added_sample,
                                                                         old_entity=entity_to_convert,
                                                                         new_entity=self.perspective_entity,
                                                                         as_numpy=False)
                rule_2_individual_relevances[original_added_sample].append(float(relevance))
        ...
        for rule in rule_2_individual_relevances:
            individual_relevances = rule_2_individual_relevances[rule]
            global_relevance = average(individual_relevances)
            rule_2_global_relevance[rule] = global_relevance

            rules_with_global_relevances = sorted(rule_2_coverage.items(), key=lambda x: x[1], reverse=True)[:10]
            return rules_with_global_relevances
